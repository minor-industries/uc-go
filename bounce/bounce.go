...
func (sim *Simulation) Tick(_, dt float64) {
	...
	const decay = 0.925
	// draw
	sim.app.Strip.Each(func(i int, led *strip.Led) {
		led.R *= decay
		led.G *= decay
		led.B *= decay
	})
	...
	balls := []*Ball{
		{
			...
			M:     2.5,
			R:     defaultRadius,
			Color: strip.Led{0, 0, brightness},
		},

		{
			...
			M:     3,
			R:     defaultRadius,
			Color: strip.Led{0, brightness, 0},
		},

		{
			...
			M:     4,
			R:     defaultRadius,
			Color: strip.Led{brightness * 0.7, 0, brightness * 0.7},
		},

		{
			...
			M:     8,
			R:     defaultRadius,
			Color: strip.Led{brightness, 0, 0},
		},
	}
